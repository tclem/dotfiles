...
- **ADRs for major decisions.** All my projects use Architecture Decision Records. Major technical decisions (architecture changes, new dependency patterns, public API changes, hard-to-reverse choices) get a formal ADR. Draft the ADR and commit it — I'll handle getting it reviewed by the team before implementation proceeds.
  - **Filenames.** Prefer `YYYY-MM-DD-kebab-name.md` (e.g. `2026-05-16-search-tokenizer.md`). Sequential numbering (`0067-foo.md`) collides when multiple agents author ADRs in parallel; date prefixes don't, and they sort the same way. If a repo already has a sequential-numbering convention with many existing files, follow the existing convention rather than mixing styles — but for new ADR directories, start with the date-prefixed pattern.
