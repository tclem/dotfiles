...
For cross-repo reading at GitHub, `gh blackbird` (see the `blackbird` skill) jumps straight to symbols and usages without cloning. For local source, `rg` inside the registry cache, `GOMODCACHE`, gem path, or `node_modules` is usually faster than a web fetch.
