...
For cross-repo reading at GitHub, `gh blackbird` (see the `blackbird` skill) jumps straight to symbols and usages without cloning. For local source, grep the registry cache, `GOMODCACHE`, gem path, or `node_modules` — usually faster than a web fetch.
